...
class FokkerPlanck(Equation):
    def __init__(self, parameters, lamb, mu, g, f):
    ...
    def next_position(self, pre_n, pre_x, delta_B, delta_t):
        lamb = self.lamb(pre_n)*delta_t
        mu = self.mu(pre_n)*delta_t
        rand = torch.rand_like(pre_n)
        delta_n = (rand<lamb).float() - (rand>1-mu).float()
...
class FeynmanKac(FokkerPlanck):
    def __init__(self, parameters, lamb, mu, g, f, U):
        super(FeynmanKac,self).__init__(parameters,lamb,mu,g,f)
        self.U = U
